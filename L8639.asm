;LAB 10 PROGRAM 8
;Write a program to find the sum of the series 1×2^2+2×3^2+3×4^2+... up to the terms entered
;by the user and display the result in decimal format. (also try to display the sum in Hex
;format)
...
.DATA
    INPUT DB 3,?,3 DUP ('?')
    PROMPT1 DB "Enter upto 2 digit number (upto 39): $"
...
L1:     MOV DL,[BX]     ;MOV CURRENT BYTE TO DL 
        ...
        ;GET SERIES  1x2^2 + 2x3^2 + 3x4^2 + ..... + n x (n+1)
        ;formula to verify sum = n (n+1)[{3n^2+11n+4}/12 + 1/2]
L2:     MOV AX,CX
        INC CX
        MUL CX
        MUL CX
